"""BC-ResNet ablation — no SpecAugment, no Mixup. Single seed=42.

Mirrors the Phase 1 BC-ResNet training recipe (build_bc_resnet architecture, Adam
...
Run:
    ...
    /Users/ad/codes/.venv310/bin/python3 scripts/train_bcresnet_no_specaug.py
"""
...
from bc_resnet import build_bc_resnet  # noqa: E402
...
TFLITE_PATH = os.path.join(PROJECT_ROOT, 'models', 'tflite', 'bcresnet_no_specaug_int8.tflite')
REPORT_PATH = os.path.join(PROJECT_ROOT, 'results', 'metrics', 'bcresnet_no_specaug_report.txt')
BEST_KERAS_PATH = os.path.join(MODELS_DIR, 'bcresnet_no_specaug_best.keras')
SAVED_MODEL_DIR = os.path.join(MODELS_DIR, 'bcresnet_no_specaug_saved_model')
...
def main():
    ...
    model = build_bc_resnet(input_shape=(40, 32, 1), num_classes=len(KEYWORDS))
    model.compile(
    ...
    header = (
        'BC-ResNet ablation — NO SpecAugment, NO Mixup\n'
        f'Seed: {SEED}\n'
        ...
        '\n'
        'Baseline reference (BC-ResNet with SpecAug+Mixup, seed=42):\n'
