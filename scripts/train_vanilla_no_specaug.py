"""Vanilla CNN ablation — no SpecAugment, no Mixup. Single seed=42.

Mirrors the Phase 1 Vanilla CNN training recipe (build_vanilla_cnn architecture, Adam
...
Run:
    ...
    /Users/ad/codes/.venv310/bin/python3 scripts/train_vanilla_no_specaug.py
"""
...
from vanilla_cnn import build_vanilla_cnn  # noqa: E402
...
TFLITE_PATH = os.path.join(PROJECT_ROOT, 'models', 'tflite', 'vanilla_no_specaug_int8.tflite')
REPORT_PATH = os.path.join(PROJECT_ROOT, 'results', 'metrics', 'vanilla_no_specaug_report.txt')
BEST_KERAS_PATH = os.path.join(MODELS_DIR, 'vanilla_no_specaug_best.keras')
SAVED_MODEL_DIR = os.path.join(MODELS_DIR, 'vanilla_no_specaug_saved_model')
...
def main():
    ...
    model = build_vanilla_cnn(input_shape=(40, 32, 1), num_classes=len(KEYWORDS))
    model.compile(
    ...
    header = (
        'Vanilla CNN ablation — NO SpecAugment, NO Mixup\n'
        f'Seed: {SEED}\n'
        ...
        '\n'
        'Baseline reference (Vanilla CNN with SpecAug+Mixup, seed=42):\n'
        '  INT8 test accuracy: 84.76%\n'
        '\n'
    )
